...
import com.example.gsyvideoplayer.model.VideoOptionModel;
import com.example.gsyvideoplayer.utils.CommonUtil;
import com.example.gsyvideoplayer.utils.Debuger;
import com.example.gsyvideoplayer.utils.StorageUtils;
import com.example.gsyvideoplayer.video.GSYBaseVideoPlayer;
...
import static com.example.gsyvideoplayer.utils.CommonUtil.getTextSpeed;
...
public abstract class GSYVideoPlayer extends GSYBaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    public ProgressListener2 progressListener;
    ...
    public void onVideoSeekTo(int duration) {

        if (GSYVideoManager.instance().getMediaPlayer() != null && duration > 0) {
        ...
        if (mCurrentState == CURRENT_STATE_PLAYING || mCurrentState == CURRENT_STATE_PAUSE) {
            try {
                ...
                if (progressListener != null) {
                    progressListener.progress(position);
                }

                Log.e("tag", "aaa===" + position);
            } catch (IllegalStateException e) {
                e.printStackTrace();
                return position;
            }
        }
        Log.e("tag", "aaabbb===" + position);
        return position;
    }
    ...
    public int getDuration() {
        try {
            ...
            Log.e("tag", "总时长aa" + duration);
            duration2 = duration;
//            progress(duration2);
            if (progressListener != null) {
                progressListener.proDuration(duration2);
            }
        ...
        Log.e("tag", "总时长aabb" + duration);

        return duration;
    }

    public void progress(ProgressListener2 progressListener) {
        this.progressListener = progressListener;
    }

    public int getDuration2() {

        Log.e("tag", "总时长aacc" + duration);
